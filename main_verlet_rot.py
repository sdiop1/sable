...
class grain :
    ...
    def __init__(self,x0,y0,vx0,vy0):
        ...
        self.thet_n = 0. #theta a t+dt
        self.thet_point_n = 0. #theta point a t+dt
...
def moment_grain(i,j):
    ...
    mom = -np.cross((R-delta)*nij,-min((mu*Kc*delta, Kt*vit_tan))*tij)
    #print(mom)
    return mom
...
def force_totale(i):
    ...
    if paroi == True :
        ...
        i.force_paroi += fp
    if gravite == True :
        fgrav = np.array([0,-g],dtype='float64')
        force += fgrav
        i.grav = fgrav
    return (force, moment)
...
def mise_a_jour(remove=True):
    ...
    '''
    for i in grains:
        f, moment = force_totale(i)
        i.force = f 
        i.pos_n = 2 * i.pos - i.pos_a + 1/m * f * dt**2
        i.vit_n = (i.pos_n - i.pos_a)/(2*dt)
        i.thet_n = 2 * i.thet - i.thet_a + moment * dt**2
        i.thet_point_n = (i.thet_n - i.thet_a)/(2*dt)
    for i in grains:
        i.pos, i.pos_a = i.pos_n, i.pos
        i.thet, i.thet_a = i.thet_n, i.thet
        i.vit = i.vit_n
        i.thet_point = i.thet_point_n
        #i.thet += 100*dt
        if i.pos[1] < y_min :
            if remove == True:
                grains.remove(i)
